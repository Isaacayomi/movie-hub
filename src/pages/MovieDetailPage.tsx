...
export default function MovieDetailPage() {
  ...
  } = useMovieSimilar(movieId, primaryGenre);
  ...
  if (isErrorDetail || !movie) {
    return (
      <div className="container mx-auto px-6 py-12">
        <ErrorState message="Failed to load movie details." onRetry={handleRetry} />
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-6 py-8 space-y-10">
      ...
      <div className="flex flex-col md:flex-row gap-10">
          ...
          <div className="relative rounded-3xl overflow-hidden bg-slate-100 border border-slate-200/50 shadow-xl shadow-slate-200/50 aspect-[2/3]">
            {posterUrl ? (
              ...
              <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 p-8">
                ...
                <span className="text-sm font-bold text-center leading-snug">{movie.title}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 space-y-6">
          ...
          <div className="space-y-2.5">
            <h3 className="text-lg font-bold text-slate-900 tracking-tight">Overview</h3>
            ...
            <table className="w-full text-left border-collapse text-sm">
              <tbody>
                <tr className="border-b border-slate-50">
                  <td className="px-5 py-4 font-bold text-slate-400 w-1/3 text-xs uppercase tracking-wider">Release Date</td>
                  <td className="px-5 py-4 text-slate-700 font-medium">{movie.release_date || 'N/A'}</td>
                </tr>
                <tr className="border-b border-slate-50">
                  <td className="px-5 py-4 font-bold text-slate-400 text-xs uppercase tracking-wider">Director</td>
                  <td className="px-5 py-4 text-slate-700 font-semibold">{movie.director || 'N/A'}</td>
                </tr>
                <tr className="border-b border-slate-50">
                  <td className="px-5 py-4 font-bold text-slate-400 text-xs uppercase tracking-wider">Top Cast</td>
                  <td className="px-5 py-4 text-slate-700 font-medium">{movie.actors || 'N/A'}</td>
                </tr>
                <tr className="border-b border-slate-50">
                  <td className="px-5 py-4 font-bold text-slate-400 text-xs uppercase tracking-wider">Language</td>
                  <td className="px-5 py-4 text-slate-700 font-medium">{movie.language || 'N/A'}</td>
                </tr>
                <tr className="border-b border-slate-50">
                  <td className="px-5 py-4 font-bold text-slate-400 text-xs uppercase tracking-wider">Budget</td>
                  <td className="px-5 py-4 text-slate-700 font-medium">N/A</td>
                </tr>
                <tr>
                  <td className="px-5 py-4 font-bold text-slate-400 text-xs uppercase tracking-wider">Revenue (Box Office)</td>
                  <td className="px-5 py-4 text-slate-700 font-medium">{movie.boxOffice || 'N/A'}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <section className="space-y-6 pt-6 border-t border-slate-100">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Similar Movies</h2>
          <p className="text-xs text-slate-400 font-medium">Recommended based on this title's genre</p>
        </div>
