...
export default function SearchPage() {
  ...
  const hasActiveFilters = rawQuery || genre || year || rating || sortBy !== 'popularity.desc';
    ...
    <div className="container mx-auto px-6 py-8 space-y-8">
      ...
      <div className="flex items-center justify-between border-b border-slate-100 pb-2">
        <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">
          Results: {movies.length === 0 && !isLoading ? '0' : data?.total_results || '0'} movies found
        </span>
      </div>
      ...
      {isLoading ? (
        ...
        <ErrorState message="An error occurred while fetching search results." onRetry={refetch} />
      ) : movies.length === 0 ? (
        <EmptyState 
