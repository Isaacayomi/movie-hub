...
export function mapOMDbMovieDetail(raw: OMDbMovieDetail): MovieDetail {
  const genres = raw.Genre && raw.Genre !== 'N/A' 
    ? raw.Genre.split(',').map((g) => g.trim()) 
    : [];
  
  const rating = raw.imdbRating && raw.imdbRating !== 'N/A' ? parseFloat(raw.imdbRating) : 0;
  const votes = raw.imdbVotes && raw.imdbVotes !== 'N/A' ? parseInt(raw.imdbVotes.replace(/,/g, ''), 10) : 0;
  const isAdult = raw.Rated ? (raw.Rated.includes('R') || raw.Rated.includes('NC-17')) : false;
...
export const omdbApi = {
  ...
  getMoviesWithDetails: async (query: string, page = 1): Promise<{ results: MovieDetail[]; total_results: number }> => {
